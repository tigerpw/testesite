...
if (isset($_POST['btn_send']))
{
	...
	$mail->Username = 'c.g233@web.de'; // SMTP username
	$mail->Password = 'aA216600727541b'; // SMTP password
	$mail->From     = 'c.g233@web.de';//email id of the person 
	$mail->AddAddress("c.g233@web.de");//my email id
...
{ // Putting everything in this if block.
	...
	else
	{
		//username exists
		$profile_data 	= array();
		$account_id 	= $accounts->fetch_info('AccountId', 'Username', $username); //get accountsid from username in Url.
...
<div id="header"></div>	
	    ...
	    <div id="container">
			<?php include 'includes/menu.php'; ?>
				...
				<h2_hl><?php echo $profile_data['Username']; ?></h2_hl>
				<h2_std> last logged in on: <?php echo date('F j, Y', $account['LastOnlineUtc'])?></h2_std><br><br>
	    		
				<?php if(!empty($profile_data['Username']) && $account['Username'] == $profile_data['Username']){?>
				<?php 
				$general->logged_out_protect();?>
				<h2_std>AccountName: </h2_std><h2_hl> <?php echo $account['Username']; ?></h2_hl><br>				
				<h2_std>Registered E-Mail: </h2_std><h2_hl> <?php echo $account['Email']; ?></h2_hl><br>	
				<h2_std>AccessLevel: </h2_std><h2_hl> <?php echo $account['AccessLevel']; ?></h2_hl><br>	
